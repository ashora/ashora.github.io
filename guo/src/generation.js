...
function getBuyButtonHTML(data) {
    ...
    if (data) {
        ...
        return `<span class="resource-count">免费</span>`;
    }
}
...
function getCardHTML(resource, value) {
    ...
    if (resource == "stairs" || resource == "upgrade") {
        topHTML = (resource == "stairs") ? `<span class="name label">农历年</span>` : `<span class="name label">价值提升</span>`;
        bottomHTML = (resource == "stairs") ? `<div class="description label">转眼间一年就过去了.</div>` :
            `<div class="description label">广大群众喜迎涨价.</div>`;
    } else if (isNaN(parseInt(value))) {
        topHTML = `<span class="name label">${resource} ${value}</span>`;
        const timer = getTool({resource: resource, value: value}).timer;
        bottomHTML = `<div class="description label">开箱后演出间隔 ${timer}s.</div>`;
    } else {
